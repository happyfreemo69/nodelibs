...
function Logger(options, config){
    ...
    this.config = config;
    ['logLvl', 'lgs_processTimeout', 'lgs_minBlockLength', 'lgs_minFullLength', 'lgs_unlockAfter'].forEach(x=>{
    ...
    this.logProc = new LogProc({
        ...
        minBlockLength:config.lgs_minBlockLength,
...
Logger.prototype._log = function(lvl, s){
    ...
    if(this.lvls[lvl] >= this.config.logLvl){
        ...
        return self.logProc.write([s,headers]);
    }
};
