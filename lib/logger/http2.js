...
var httpOrHttps = require('../si').getProtocolModule;
/**
 * [Logger description]
 * @param {[type]} options {
 *   host: mandatory 
...
Logger.prototype.write = function(s){
    if(this.lvls['INF'] >= this.config.logLvl){
...
Logger.prototype.dbg = function(){
    if(this.lvls['DBG'] >= this.config.logLvl){
		var s = util.format.apply(console, arguments);
		this._log('DBG', s);
	}
};
...
Logger.prototype.err = function(){
    if(this.lvls['ERR'] >= this.config.logLvl){
		var s = util.format.apply(console, arguments);
		this._log('ERR', s);
	}
};
...
Logger.prototype.cri = function(){
    if(this.lvls['CRI'] >= this.config.logLvl){
		var s = util.format.apply(console, arguments);
		this._log('CRI', s);
	}
};
...
Logger.prototype.sta = function(){
    if(this.lvls['STA'] >= this.config.logLvl){
		var s = util.format.apply(console, arguments);
		this._log('STA', s);
	}
};
...
Logger.prototype._dequeue = function(tasks, cbk){
    var self = this;
    var req = httpOrHttps(this.options.protocol).request({
        method:'POST',
        path:'/'+this.options.appName+'/bulks',
        hostname:this.options.host,
        port:this.options.port,
        headers:{
            Host: this.options.upstream
        }
    }, function(res){
