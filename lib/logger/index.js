...
/**
 * sends a logging message to papertrails if winston is down
 * @param  {[type]} url    mandatory, webhook url to slack (check 
 * @param  {[type]} str    mandatory, string which will be logged as message 
 *                         to say papertrails is down %date% is replaced by the current date
 * @param  {[type]} logger used to log if event the notification fails...
 */
function pptOnError(url, str, logger){
    var request = require('request');
    var text = str.replace('%date%', new Date());
    var json = {"channel": "#api", "username": "webhookbot", "text": text.toUpperCase(), "icon_emoji": ":ghost:"};
    request({
        url:'https://hooks.slack.com/services/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',
        method:'POST',
        json: true,
        body: json
     },function (error, response, body) {////{ message: 'test '+config.winston_exitMsg.replace('%date%', new Date()) }
    ...
    if(config.winston_enable){
        ...
        this.winston = new Winston.Logger({
            ...
            exitOnError:function(){
                ...
                return config.winston_pptOnError && pptOnError(config.winston_slackUrl, config.winston_slackPptDownMessage, this);
            }
        });
        //log the error
