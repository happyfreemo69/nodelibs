 ...
 */
function pptOnError(url, str, e, logger){
    var request = require('request');
    var text = str.replace('%date%', new Date());
    var json = {"channel": "#api", "username": "webhookbot", "text": text.toUpperCase()+'\n'+JSON.stringify(e), "icon_emoji": ":ghost:"};
    request({
        ...
        this.winston = new Winston.Logger({
            ...
            exitOnError:function(e){
                if(config.winston_pptOnError && self._winstonConstruct){
                    ...
                    pptOnError(config.winston_slackUrl, config.winston_slackPptDownMessage, e, self);
                }
                return false;
            }
        });
