...
/**
 * @param  {{string: fieldInstance}} obj 
 * @return {[type]}     [description]
 */
Field.prototype.or = function(obj){
    this.orObj = obj;
    this.rules.push({
        ok:rules.anything
    });
...
Field.prototype.anyNws = function(){
    this.rules.push(rules.anyNonWs);
    return this;
};
...
Field.prototype.validate = function(key, obj){
    ...
    if(ok(res)){
        ...
        if(this.subObj){
            ...
            var subs = [];
            var success = Object.keys(this.orObj).some(k=>{
                var sub = this.orObj[k].validate(k, obj[k]);
                if(ok(sub)){
                    return sub;
                }
                subs.push(sub)
            ...
            if(!success && subs.length){
                subErrors = [subs.join(' OR ')];
            }
        }
