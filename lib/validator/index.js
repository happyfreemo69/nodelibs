...
exports.html = function(tags){
    return V.str().func(function(key,val){
        ...
        let allowed = Object.keys(tags).filter(x=>tags[x][0]!=='all')
        let allowedAttributes = {};
        allowed.map(x=>{
            if(tags[x][0]!=='all') allowedAttributes[x] = tags[x];
        })
        let clean = sanitizeHtml(val,{allowedTags:allowedTags, allowedAttributes:allowedAttributes})
        clean = clean.replace(' ','');
        if(clean !== val.replace(' ','')){
            return key+'html contains non-allowed tags or properties';
        }
        return true;
