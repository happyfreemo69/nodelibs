...
exports.html = function(tags){
    return V.str().func(function(key,val){
        ...
        allowed.map(x=>{
            if(tags[x][0]!=='all') allowedAttributes = allowedAttributes.concat(tags[x]);
        })
        ...
        if(clean.length !== val.length){ // https://github.com/cure53/DOMPurify/issues/276
            return key+' nodecontains non-allowed tags or properties';
        }
        return true;
    });
}
