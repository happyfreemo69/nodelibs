...
var DEBUG = true;
function checkKeys(jsonRef, json, prefix, skip = []){
...
function checkLinks(linksRef, linksTest, skip = []){
    checkKeys(linksRef, linksTest, 'linksLevel:', skip);
    Object.keys(linksRef).forEach(function(link){
        if (skip.indexOf(link) === -1) {
...
function checkJson(jsonRef, json, skip = []){
    checkKeys(jsonRef, json, null, skip);
    jsonRef.links && checkLinks(jsonRef.links, json.links, skip);
}
