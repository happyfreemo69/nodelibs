    ...
    bulks.forEach(function(bulk, bulkIndex){
        p = p.then(function(){
            ...
            return Promise.all(dfds).then(function(){
                ...
                rl.write(count+'/'+collection.length+' (avg='+avg+'r/s) endsAt('+d.getHours()+'h'+d.getMinutes()+'m)');
...
function findIt(query, nbElemPerBulk, it, done, onBulkEnd){
    ...
    var nextBulk = function(cursor, nbElemPerBulk, it, onBulkEnd){
        ...
        return p.then(function(){
            ...
            return Promise.all(dfds).then(function(){
                var now = Date.now();
                var elapsedSeconds = (now-dateRef)/1000;
                count += dfds.length;
                var avg = Math.round(count/(elapsedSeconds))||1;
                var d = new Date(beginning+(dfds.length-count)/avg*1000);
                if(now - previousTs > 1000){
                    previousTs = now;
                    readline.cursorTo(rl.output, 0);
                    rl.write('bulk:'+bulkIndex+' (avg='+avg+'r/s) endsAt('+d.getHours()+'h'+d.getMinutes()+'m)');
                }
                return onBulkEnd();
            })
        }).then(function(){
                ...
                return Promise.all(lines.map(it)).then(function(){
                    return onBulkEnd().then(function(){
                        ...
                        if(now - previousTs > 1000){
                            previousTs = now;
                            readline.cursorTo(rl.output, 0);
                            rl.write(count+'/'+nbLines+' (avg='+avg+'r/s) endsAt('+d.getHours()+'h'+d.getMinutes()+'m)');
                        }
